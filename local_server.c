  ...
  while (1) {
    ...
    if (rr != NULL) {
      ...
      if(localAnwserPacket.header->tag != name_wrong_res) {
        printf("receive packet from root:the last number is %d\n", localAnwserPacket.authoritySection->rData[3]);
      } else {
   ...
   while (localAnwserPacket.header->answerNum == 0 && localAnwserPacket.header->tag != name_wrong_res) {
        ...
        memcpy(udp_server_ip, ip, 8);
         udp_server_ip += 8;
        *udp_server_ip = *localAnwserPacket.authoritySection->rData + '0';
         udp_server_ip -= 8;
